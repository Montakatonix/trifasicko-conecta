...
import type { TarifaLuz, ConsumoElectrico, TarifaInternet, FiltrosInternet } from "./types"
...
export function calcularCosteLuz(tarifa: TarifaLuz, consumo: ConsumoElectrico): number {
  ...
  if (consumo.discriminacionHoraria && consumo.consumoPunta && consumo.consumoValle) {
    ...
    if (tarifa.precioKwhLlano && consumo.consumoLlano) {
      costeTotal += tarifa.precioKwhLlano * consumo.consumoLlano
    }
  } else {
...
export function filtrarTarifasInternet(tarifas: TarifaInternet[], filtros: FiltrosInternet): TarifaInternet[] {
...
export function calcularPotenciaRecomendada(electrodomesticos: Electrodomestico[]): number {
  ...
  return potenciasNormalizadas.find(p => p >= potenciaTotal) || potenciasNormalizadas[potenciasNormalizadas.length - 1]
}
